"""
Fit model (that includes metals) to COS spectrum
Redshift model by 37 km/s
Measure wavelengths of ISM lines - check velocity (if any)
...
def model_the_line(contw, contf, linew, linef, n=3):
    ...
    contw_only = contw[~line_in_cont] # continuum data not in line
    contf_only = contf[~line_in_cont]
    
    coefs = np.polyfit(contw_only, contf_only, n) # weight by w=1/error**2 or suchlike?
    #plt.figure()
    #plt.plot(contw, contf, label='All data', color='grey', alpha=0.2)
    #plt.plot(contw_only, contf_only, label='Continuum Data')
    #plt.plot(contw, np.polyval(coefs, contw), label='Fit')
    #plt.legend()
    #plt.show()
    ...
    g_amp = linef[len(linew)//2] - np.mean(linef[0:3])
    g_mdpt = np.mean(linew)

    g_init = models.Gaussian1D(amplitude=g_amp, mean=g_mdpt, stddev=g_fwhm)
    fit_g = fitting.LevMarLSQFitter()
    g_result = fit_g(g_init, contw, contf_reduced) # weight by weights=1/error**2 or suchlike?
    ...
    a_res.errorbar(contw, final_gauss - contf, conte,
    fmt='k.')
    a_res.set_xlabel('Wavelength [$\AA$]')
    a_res.set_ylabel('Relative flux')
    
    resid_mean = np.mean(final_gauss - contf)
    print('Mean of residuals = {:.5f}'.format(resid_mean))
...
def toggler(event):
    ...
    print('Continuum range = {:.3f}-{:.3f}'.format(cx.min(), cx.max()))
...
#for mdpt in ISM_ROI:
    #guide = np.asarray([[mdpt, 0],[mdpt,0.5e-14]])
    #subset = cos[np.where(np.logical_and(cos[:,0]>mdpt-0.8, cos[:,0]<mdpt+0.8))]
    #plot01(subset)
    #plot01(guide, colour='r', lstyle='-')
    #plt.show(block=False)
    #amplit = float(input('Line Depth (*10^-15): '))
    #stdv = float(input('FWHM (Angstroms): '))
    #plt.close()
    #x=fit_gauss(subset, amplit, mdpt, stdv)
    #break
